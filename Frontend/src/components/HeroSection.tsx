...
export default function HeroSection() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      if (svgPathRef.current) {
        ...
        tl.to(svgPathRef.current, { strokeDashoffset: 0, duration: 3, ease: 'none' }, 0);
      }
      ...
      tl.to([feeLineRef.current, subtitleLineRef.current], {
        y: -64,
        opacity: 0.35,
        filter: 'blur(2px)',
        duration: 0.7,
      ...
      tl.to(brandRef.current, { opacity: 1, y: 0, filter: 'blur(0px)', duration: 0.6, ease: 'power2.out' }, 2.75);

      // Description
      gsap.set(descRef.current, { opacity: 0, y: 8 });
      tl.to(descRef.current, { opacity: 1, y: 0, duration: 0.45, ease: 'power2.out' }, 3.15);

      // CTA
      gsap.set(ctaRef.current, { opacity: 0, y: 12 });
      tl.to(ctaRef.current, { opacity: 1, y: 0, duration: 0.4, ease: 'power2.out' }, 3.4);
      ...
      if (dot) {
        gsap.to(dot, { y: 44, repeat: -1, duration: 1.8, ease: 'power1.inOut', yoyo: true });
      }
    }, sectionRef);
  ...
  return (
    ...
    >
      ...
      <svg className="absolute inset-0 w-full h-full pointer-events-none z-0" style={{ opacity: 0.035 }} preserveAspectRatio="none" viewBox="0 0 1440 900">
        <polyline
          ref={svgPathRef}
      ...
      <div className="relative z-10 text-center flex flex-col items-center">
        {/* Fee line */}
        <div ref={feeLineRef} className="font-fraunces text-negative text-[44px] md:text-[64px] leading-none tracking-tight" style={{ fontVariationSettings: "'opsz' 144, 'wght' 700", letterSpacing: '-0.02em' }}>
        ...
        {/* Subtitle */}
        <div ref={subtitleLineRef} className="font-syne text-text-secondary text-[16px] md:text-[18px] mt-3 flex flex-wrap justify-center gap-x-1.5">
          {['lost', 'to', 'hidden', 'fees.', 'Every', 'year.'].map((word, i) => (
        ...
        <div ref={brandRef} className="mt-8">
          <h1
            ...
            style={{ fontVariationSettings: "'opsz' 144, 'wght' 800", letterSpacing: '-0.03em' }}
          >
            ArthSaathi
          </h1>
          <p className="font-syne text-text-tertiary text-[15px] mt-1.5">(अर्थसाथी)</p>
        </div>

        {/* Description */}
        <p ref={descRef} className="font-syne text-text-secondary text-[17px] max-w-[440px] text-center leading-relaxed mt-5">
        ...
        {/* Microcopy */}
        <div ref={microRef} className="font-syne text-text-muted text-[13px] mt-4">
          No signup · 30 seconds
        </div>
      </div>

      {/* Scroll indicator */}
      <div ref={scrollRef} className="absolute bottom-7 left-1/2 -translate-x-1/2 flex flex-col items-center">
