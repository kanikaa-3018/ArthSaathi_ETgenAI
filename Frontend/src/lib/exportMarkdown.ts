/**
 * Structured Markdown report — shared source for `.md` download and PDF (via exportPdfFromMarkdown).
 * Aligned with PR #13 (kanikaa-3018/ArthSaathi_ETgenAI#13).
 */
...
function table(headers: string[], rows: string[][]): string {
  ...
  if (!rows.length) return [h, sep, "| *No rows* |"].join("\n");
  const r = rows.map((row) => `| ${row.map((c) => cell(c)).join(" | ")} |`).join("\n");
  return [h, sep, r].join("\n");
}

/** TER: API uses fraction (e.g. 0.021); mocks may use whole percent — same rule as FundTable. */
function terAsDisplayPercent(ter: number): number {
  return ter > 0.2 ? ter : ter * 100;
}

function weightedTerDisplay(ter: number): string {
  return `${terAsDisplayPercent(ter).toFixed(2)}%`;
}
...
---`);
  ...
  const breakdownEntries = Object.entries(data.health_score.breakdown ?? {});
  ...
  const fundRows = data.funds.map((f) => [
    ...
    f.xirr.display,
    `${terAsDisplayPercent(f.expense.estimated_ter).toFixed(2)}%`,
  ...
  parts.push(
    `\n## Expense drag\n\n${table(
      ...
      [
        ...
        ["Weighted avg TER", weightedTerDisplay(data.expense_summary.weighted_average_ter)],
      ],
    )}`,
  );
  ...
  if (oa.matrix?.length) {
    const top = [...oa.matrix].sort((a, b) => Number(b.overlap) - Number(a.overlap)).slice(0, 50);
    parts.push(
  ...
  const prov = data.rebalancing_plan.llm_provider ?? data.rebalancing_plan.ai_provider;
  const modelBit = data.rebalancing_plan.llm_model ? ` · ${data.rebalancing_plan.llm_model}` : "";
  parts.push(
    `\n## AI rebalancing plan\n\n_${data.rebalancing_plan.ai_generated ? `AI-generated (${prov}${modelBit})` : "Rules-based"}_\n\n${data.rebalancing_plan.content.trim()}`,
  );
  ...
  if (showPlannerAndTax && taxInsights) {
    ...
    parts.push(
      `\n## Tax insights\n\n_Log in and open **Tax insights** in the app for illustrative estimates (requires API)._`,
    );
  }
