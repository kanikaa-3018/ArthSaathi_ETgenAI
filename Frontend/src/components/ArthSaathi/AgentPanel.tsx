...
const agentDefs = [
  ...
  },
  {
    ...
    delay: 2100,
    duration: 3400,
    runMsg: "Fetching live NAVs for 6 schemes...",
    doneMsg: "All NAVs fetched",
    severity: "success" as const,
  },
  {
    id: "returns",
    ...
    delay: 5500,
    duration: 800,
    runMsg: "Computing XIRR for 6 funds...",
    doneMsg: "Portfolio XIRR: 12.84%",
    severity: "success" as const,
  },
  {
    id: "overlap",
    ...
    delay: 5500,
    duration: 1200,
    runMsg: "Analyzing stock overlap...",
    doneMsg: "45.2% overlap: HDFC Top 100 ↔ ICICI Bluechip",
  ...
  },
  {
    ...
    delay: 5500,
    duration: 900,
    runMsg: "Analyzing expense ratios...",
    doneMsg: "₹40,697/year lost to expense fees",
  ...
  },
  {
    ...
    delay: 7000,
    duration: 1000,
    runMsg: "Projecting wealth trajectories...",
    doneMsg: "₹22.6 L gap over 10 years",
    severity: "critical" as const,
  },
  {
    id: "health",
    ...
    delay: 8000,
    duration: 500,
    runMsg: "Calculating health score...",
    doneMsg: "Health Score: 41/100 — Needs Attention",
    severity: "warning" as const,
  },
  {
    id: "advisor",
    ...
    delay: 8500,
    duration: 2000,
    runMsg: "Generating AI rebalancing plan...",
    doneMsg: "Plan ready — 3 actions identified",
    severity: "success" as const,
  },
];
...
export function AgentPanel({
    ...
    agentDefs.forEach((def) => {
      schedule(() => {
        ...
        setAgents((prev) =>
          prev.map((a) =>
            a.id === def.id
              ? { ...a, status: "running", message: def.runMsg }
              : a,
          ),
        );
      }, def.delay);
      ...
      schedule(() => {
        if (runId !== runIdRef.current) return;
        const elapsed = ((Date.now() - startTime - def.delay) / 1000).toFixed(
          1,
        );
        setAgents((prev) =>
          prev.map((a) =>
        ...
        schedule(() => {
          if (runId !== runIdRef.current) return;
          setAgents((prev) =>
            prev.map((a) => (a.id === def.id ? { ...a, flash: false } : a)),
          );
        }, 600);
  ...
  useEffect(() => {
    ...
    const allDone =
      agents.length > 0 &&
      agents.every((a) => !["queued", "running"].includes(a.status));
  ...
  const completed = agents.filter(
    (a) => !["queued", "running"].includes(a.status),
  ).length;
  ...
  const StatusIcon = ({ status }: { status: AgentState["status"] }) => {
    switch (status) {
      ...
      case "success":
        return <Check size={14} style={{ color: "hsl(var(--positive))" }} />;
      case "warning":
        return (
      ...
      case "critical":
        return (
          <AlertCircle size={14} style={{ color: "hsl(var(--negative))" }} />
        );
    }
  };
      ...
      <div className="space-y-1">
        {agents.map((agent) => (
          ...
          >
            <span
              ...
              style={{
                background:
                  agent.status === "queued"
                    ? "transparent"
                    : dotColor(agent.status),
                border:
                  agent.status === "queued"
                    ? "1.5px solid hsl(var(--text-tertiary))"
                    : "none",
                boxShadow:
            ...
            />
            <span
              className="font-body text-sm font-medium w-32 flex-shrink-0"
              style={{
                color:
                  agent.status === "queued"
